...
if [ "${interactive}" == "1" ]; then
  sudo apt install nmap screen bzip2 psmisc htop mc grc iputils-ping zsh autojump jq python3-pygments httpie molly-guard || ( echo "Installation failed... Exiting." && exit 1 )
  sudo apt install ruby-albino
else
  sudo apt -y install nmap screen bzip2 psmisc htop mc grc iputils-ping zsh autojump jq python3-pygments httpie molly-guard || ( echo "Installation failed... Exiting." && exit 1 )
